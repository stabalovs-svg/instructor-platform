-- Derive instructor access and reminders from server-side subscription dates.
...
begin
  ...
  v_writable := v_subscription.status in ('trial', 'active')
    and (v_subscription.valid_until is null or current_date <= coalesce(v_subscription.grace_until, v_subscription.valid_until));
  v_export_allowed := v_writable
    or (v_subscription.export_access_until is not null and current_date <= v_subscription.export_access_until);

  v_kind := case
    when not v_writable and v_subscription.cancellation_notice_at is not null then 'termination'
    when not v_writable then 'expired'
    when v_subscription.valid_until is not null and v_subscription.valid_until - current_date <= 3 then 'three_days'
    when v_subscription.valid_until is not null and v_subscription.valid_until - current_date <= 7 then 'seven_days'
    else null
  end;
  ...
  return jsonb_build_object(
    ...
    'paidAt', v_subscription.paid_at,
    'periodMonths', v_subscription.period_months,
    'paidAmount', v_subscription.paid_amount,
    'validUntil', v_subscription.valid_until,
    'graceUntil', v_subscription.grace_until,
    'daysRemaining', case when v_subscription.valid_until is null then null else v_subscription.valid_until - current_date end,
    'calendarWritable', v_writable,
    'exportAllowed', v_export_allowed,
    ...
    'notification', case when v_notification.id is null then null else jsonb_build_object(
      ...
      'effectiveOn', v_notification.effective_on,
      'dismissedAt', v_notification.dismissed_at
    ) end
  );
end;
$$;
...
comment on function public.get_my_subscription_access() is
  'Returns server-derived access and creates at most one applicable daily subscription reminder.';
